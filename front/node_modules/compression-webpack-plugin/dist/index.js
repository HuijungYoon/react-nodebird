...
class CompressionPlugin {
  ...
  *getTask(compilation, assetName) {
  ...
  afterTask(compilation, task, weakCache) {
    ...
    if (!weakOutput) {
      weakOutput = new _RawSource.default(output);
  ...
  apply(compiler) {
    ...
    const compressionFn = async (compilation, CacheEngine, assets) => {
      const assetNames = Object.keys(typeof assets === 'undefined' ? compilation.assets : assets).filter(assetName => matchObject(assetName));
    ...
    if (CompressionPlugin.isWebpack4()) {
      ...
      compiler.hooks.compilation.tap(pluginName, compilation => {
        ...
        compilation.hooks.statsPrinter.tap(pluginName, stats => {
          stats.hooks.print.for('asset.info.compressed').tap('compression-webpack-plugin', (minimized, {
            green,
            formatFlag
          }) => // eslint-disable-next-line no-undefined
