...
class Webpack4Cache {
  ...
  async get(task) {
    ...
    try {
      cachedResult = await _cacache.default.get(this.cacheDir, task.cacheIdent);
    } catch (ignoreError) {
  ...
  async store(task) {
    return _cacache.default.put(this.cacheDir, task.cacheIdent, JSON.stringify(task.output));
  }
